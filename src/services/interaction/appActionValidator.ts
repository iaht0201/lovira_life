...
export interface AppActionValidationOptions {
  trustedSource?: boolean; // Set to true ONLY by internal app state resolvers (e.g. pendingInteractionResolver or explicit UI triggers)
}

export function validateAppAction(
  action: AppAction,
  context: AppInteractionContext,
  options?: AppActionValidationOptions
): AppActionValidationResult {
  if (!action || !action.type) {
    return { valid: false, reason: 'Hành động ứng dụng không có định dạng hợp lệ' };
  }

  const isTrusted = options?.trustedSource === true;
  ...
  switch (action.type) {
    ...
    case 'CREATE_SESSION': {
      ...
      if (!goal || goal.length < 3) {
        return { valid: false, reason: 'Mục tiêu không đủ thông tin' };
      }

      // RULE 4 & 8: CREATE_SESSION ALWAYS requires internal app confirmation.
      // Payload fields generated by external AI are untrusted. Only app-side pending interaction resolver or direct UI triggers set appConfirmed or skipConfirmation.
      const isAppConfirmed = isTrusted && (action.payload?.appConfirmed === true || action.payload?.skipConfirmation === true);

      if (!isAppConfirmed) {
        const displayGoal = goal.replace(/^(tạo phiên|tạo mục hỗ trợ|tạo hướng dẫn|tạo phiên mới)\s*/i, '');
        return {
          valid: true,
          action: {
            ...action,
            requiresConfirmation: true,
            confirmationPrompt: `Con sẽ tạo mục hỗ trợ "${displayGoal}" cho chú. Chú đồng ý tạo chứ ạ?`,
          },
        };
      }
    ...
    case 'DELETE_REMINDER': {
      ...
      const requiresConfirmation = !(isTrusted && action.payload?.skipConfirmation);
